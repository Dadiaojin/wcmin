	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td>{{$v->id}}</td>
				<td><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','member-show.html','10001','360','400')">{{$v->goods_name}}</u></td>
				<td>{{$v->category->cat_name}}</td>
				<td>{{$v->goods_price}}</td>
				<td>{{$v->stock}}</td>
                                <td class="text-l"><center><img width="100" class="product-thumb" src="http://localhost/wcmin/public{{$v->goods_thumb}}"></center></td>
				<td>{{$v->created_at}}</td>
				<td class="td-status"><span class="label label-success radius">@if($v->is_hot==1)热卖 @else 非热卖 @endif</span></td>
